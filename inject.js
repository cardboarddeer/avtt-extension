...
async function getCombatState() {
  ...
  if (
    !window.__AVTT_SAVE_DIAGNOSTIC_LOGGED__ &&
    Array.isArray(window.pcs) &&
    window.pcs.length
  ) {
    window.__AVTT_SAVE_DIAGNOSTIC_LOGGED__ = true;

    const diagnosticPc =
      window.pcs[0];

    console.log(
      "AVTT saving throw diagnostic:",
      {
        name:
          diagnosticPc?.name,

        characterId:
          diagnosticPc?.characterId,
        ...
        abilities:
          diagnosticPc?.abilities,

        savingThrows:
          diagnosticPc?.savingThrows,

        modifiers:
          diagnosticPc?.modifiers,

        stats:
          diagnosticPc?.stats,

        proficiencyBonus:
          diagnosticPc?.proficiencyBonus,

        directSaves: {
          strSave:
            diagnosticPc?.strSave ??
            diagnosticPc?.strengthSave,

          dexSave:
            diagnosticPc?.dexSave ??
            diagnosticPc?.dexteritySave,

          conSave:
            diagnosticPc?.conSave ??
            diagnosticPc?.constitutionSave,

          intSave:
            diagnosticPc?.intSave ??
            diagnosticPc?.intelligenceSave,

          wisSave:
            diagnosticPc?.wisSave ??
            diagnosticPc?.wisdomSave,

          chaSave:
            diagnosticPc?.chaSave ??
            diagnosticPc?.charismaSave
        }
      }
    );
  }
...
async function avttModifySelectedTokenHp(
  ...
  if (options.amountPrompt) {
    const promptedAmount =
      await avttShowPrompt({
        title:
          options.amountPrompt.title ??
          "Enter Amount",

        label:
          options.amountPrompt.label,

        placeholder:
          options.amountPrompt.placeholder,

        defaultValue:
          options.amountPrompt.defaultValue ??
          requestedAmount,

        min:
          options.amountPrompt.min,

        max:
          options.amountPrompt.max,

        okText:
          options.amountPrompt.okText,

        cancelText:
          options.amountPrompt.cancelText
      });
